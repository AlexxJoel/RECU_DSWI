...
def lambda_handler(event, _context):
    ...
    try:
        ...
        query = """ DELETE FROM recu_people WHERE id = %s """
        result = transaction_db(conn, query, (id,))

        if result == "Deleted":
            return {"statusCode": 200, "body": json.dumps({"data": "Person deleted successfully"})}

        return {"statusCode": 500, "body": json.dumps({"error": "Error deleting person"})}
